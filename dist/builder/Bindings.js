    ...
    while (index++ < (data.length - 1)) {
        switch (data[index]) {
            ...
            case "'":
                if (openCount <= 1) {
                    if (strHandler !== '')
                        xlrq.push(onString ? `'${strHandler}'` : strHandler);
                    onString = !onString;
                    strHandler = "";
                }
                else
...
function parser(data) {
    ...
    if (JSON.stringify(data) !== JSON.stringify(bak))
        data = parser(data);
    return data;
}
/**
 ...
 */
function bindingSyntaxHandler(data) {
    let arr = [];
    const strLexer = lexer(data);
    for (const value of strLexer) {
        const $ = lexer(value);
        if ($.length > 1)
            arr.push(bindingSyntaxHandler(value));
        else
            arr.push(value);
    }
    ;
 ...
 */
function BindingsHandle(bindings) {
    const jsonUIBindings = [];
...
exports.BindingsHandle = BindingsHandle;
